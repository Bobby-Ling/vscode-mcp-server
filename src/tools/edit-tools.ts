...
export function registerEditTools(server: FastMCP): void {
    server.addTool({
        ...
        description: `Creates new files or completely rewrites existing files.
        ...
        execute: async ({ path, content, overwrite = false, ignoreIfExists = false }) => {
    ...
    server.addTool({
        ...
        description: `Replaces specific lines in existing files with exact content validation.
        ...
        execute: async ({ path, startLine, endLine, content, originalCode }) => {
